...
# 9b. Keeping the docs honest — run the verifier

The documents cite specific code lines. When you change code, those citations go stale, and a stale doc is how the *next* session builds the wrong thing.

```bash
python implementationdoc/verify_docs.py      # from the repo root
```

It checks five things: every relative link resolves · every `file:line` reference is in range · twelve specific claims the docs make about the code still hold · no stale/contradictory scope language · every work package appears in both `AGENT_GUIDE.md` and `PROGRESS.md`.

**Run it at the start and end of every session.** Exit code 0 means clean.

**One thing that will surprise you:** the claim checks assert that **the bugs still exist as described**. So when you fix a bug, its claim **will fail** — that is correct and expected. It means "the code changed, go update the doc." When a bug is genuinely fixed:

1. Update or mark resolved the doc section that describes it.
2. Delete that entry from the `CLAIMS` list in `verify_docs.py`.
3. **Never weaken a check just to make it pass.**

Two errors were already caught and fixed this way: a wrong line citation for `hold_duration`, and a leftover sentence claiming the paper-trading feature was being removed — which contradicted the keep-and-convert scope decision and would have made an agent delete a live feature.

(Note: the contradiction check matches literal strings, so don't quote a banned phrase verbatim in a doc, even to explain it — describe it instead. The checker cannot tell the difference.)
...
- [ ] `python implementationdoc/verify_docs.py` exits 0 (or you updated the docs and `CLAIMS` for what you fixed)
