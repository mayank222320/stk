...
## 6. Manual trigger — why it currently finds nothing

You mentioned you also trigger a news fetch manually. That path is broken in a way that looks exactly like "news is late", and it's worth fixing before anything else here.
...
The automatic scan runs **every 5 minutes all day** and marks every article it sees as processed. So when you trigger manually, almost everything is already in `processed_news` — you get zero candidates, and the endpoint returns `{"status": "scan complete"}` with no articles and no explanation.

**Your manual trigger can essentially never find anything.** Not because the news is late, but because the background job already consumed it and the manual path has no way to say "show me anyway".
